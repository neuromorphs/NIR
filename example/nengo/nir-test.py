...
n = nir.NIRGraph(
    nodes=[
        ...
        nir.Affine(weight=np.array([[8,2,10], [14,3,14]]).T, 
                   bias=np.array([1,2])),
        nir.LIF(tau=np.array([1]*2), r=np.array([1]*2), 
                   v_leak=np.array([0]*2), v_threshold=np.array([1]*2)),
        ...
        for i, obj in enumerate(n.nodes):
            if isinstance(obj, nir.Input):
                ...
                if swap_linear_order:
                    weights = weights.T
                w = nengo.Node(lambda t, x, obj=obj: weights @ x + obj.bias, size_in=weights.shape[1], size_out=weights.shape[0], label=f'({weights.shape[0]}x{weights.shape[1]})')
                nengo_map.append(w)
